...
namespace EsriTestApp
{
    ...
    {
        //public static SqlGeography myGeo { get; set; }
        //public static List<BeatSeg> beatsegs = new List<BeatSeg>();
        public static List<esriData> segPolygons = new List<esriData>();

        /*public static void addSegment(BeatSeg geo)
        {
            BeatSeg bs = beatsegs.Find(delegate(BeatSeg find){
                return find.BeatSegID == geo.BeatSegID;
            });
            if (bs == null)
            {
                beatsegs.Add(geo);
        ...
        /*
        public static bool FindOnBeat(string beatID, SqlGeography sg)
        {
            bool onBeat = false;
            var blist = from bsegs in beatsegs
                        where bsegs.BeatSegID == beatID
                        select bsegs;
            foreach (BeatSeg bseg in blist)
            {
                if (bseg.geo.STBuffer(10).STIntersects(sg))
                {
                    onBeat = true;
                }
            }
        ...
        public static bool checkIntersects(SqlGeography geo)
        {
            bool inside = false;

            if (myGeo.STBuffer(10).STIntersects(geo))
            {
                inside = true;
            }
        ...
        public static bool checkSegIntersection(string bsID, SqlGeography geo)
        {
            bool inside = false;

            BeatSeg bs = beatsegs.Find(delegate(BeatSeg find) { return find.BeatSegID == bsID; });
            if (bs != null)
            {
                if (bs.geo.STBuffer(10).STIntersects(geo))
                {
                    inside = true;
                }
            }
        ...
        public static bool checkEsriIntersect(double lat, double lon, string segID)
        {
            ...
            var spList = from sp in segPolygons
                         where sp.BeatSegID == segID
                         select sp;
            foreach (esriData ed in spList)
            {

...
namespace FPSService.TowTruck
{
    ...
    public class TowTruck
    {
        ...
        public void TowTruckGPSUpdate()
        {
            ...
            {
                inYard = false;
                bool OnBeat = BeatData.EsriBeats.checkEsriIntersect(this.GPSPosition.Lat, this.GPSPosition.Lon, beatNumber);
                ...
                if (OnBeat == true)
                {
                    ...
                    string segment = BeatData.EsriSegments.findBeatSegByBeat(this.beatNumber, this.GPSPosition.Lat, this.GPSPosition.Lon);
                ...
                {
                    //check inYard
                    string YardName = BeatData.EsriYards.findYard(this.GPSPosition.Lat, this.GPSPosition.Lon);
                    ...
                    //check inDropSite
                    string drop = BeatData.EsriDropSites.findDropSite(this.GPSPosition.Lat, this.GPSPosition.Lon);
        ...
        private void wazer(Incident i, List<Assist> aList, bool close) {
            try {
                        ...
                        currentIncident.chpIncidentType.ToUpper() != "OTHER")
                {
                    ...
                    if (close)
                    {
                        string polyLine = currentIncident.lon.ToString() + "," + currentIncident.lat.ToString() + "," + currentIncident.lon.ToString() + "," + currentIncident.lat.ToString();
            ...
            try {
                ...
                if (this.currentIncident == null){
                    if (i.incidentID == null || i.incidentID == Guid.Empty)
                    {
                        ...
                        i.dispatchLocation = BeatData.EsriSegments.findBeatSeg(this.GPSPosition.Lat, this.GPSPosition.Lon);
                        this.currentIncident = i;
                        //Log that sucka
                        SQL.SQLCode sql = new SQL.SQLCode();
                        sql.logNewIncident(currentIncident);
                        cadMSG = "EI.T/FSP.L/#$%" + getLatLon() + ".U/" + this.shiftType + this.Driver.callSign + ".S/1097";
                    }
                    else
                    {
                        i.incidentDatePosted = DateTime.Now;
                        ...
                        SQL.SQLCode sql = new SQL.SQLCode();
                        sql.logNewIncident(currentIncident);
                        //this was removed. It appears we might not need it.
                        //cadMSG = "US." + this.shiftType + this.Driver.callSign + ".S/1097.II/" + this.currentIncident.CHPLogNumber + makeCADDate(DateTime.Now) + makeCADTime(DateTime.Now);
                    }
                    //update CAD with new information

                    if (!string.IsNullOrEmpty(cadMSG)) {
                        Global.cSender.sendMessage(cadMSG);
                    }
        ...
        {
            string Lat = this.GPSPosition.Lat.ToString();
            if (this.GPSPosition.Lat >= 0)
            {
                Lat = "+" + Lat;
            }
            string Lon = this.GPSPosition.Lon.ToString();
            string Head = this.GPSPosition.Head.ToString();
            return Lat + ";" + Lon + ";" + Head;
        }
            ...
            try {
                if (this.currentIncident == null)
                {
                    ...
                    else
                    {
                        ...
                        if (i.wazeID != null && i.wazeID != Guid.Empty) {
                        ...
                        currentIncident.dispatchLocation = BeatData.EsriSegments.findBeatSeg(this.GPSPosition.Lat, this.GPSPosition.Lon);
                        SQL.SQLCode sql = new SQL.SQLCode();
                        sql.logNewIncident(currentIncident);
                        wazer(currentIncident, currentAssists, false);
                    }

...
namespace FPSService
{
    ...
    public class AJAXFSPService
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (Status == "ON INCIDENT" && CurrentStatus != "ON INCIDENT") //we're firing two messages to the CAD, one for on incident, one for post assist
                    {
                        ...
                        {
                            //we should already have an incident but we need to let CAD know what's up
        ...
        {
            List<string> freeways = BeatData.EsriBeats.freeways;
            JavaScriptSerializer js = new JavaScriptSerializer();
        ...
        {
            if (BeatData.EsriBeats.beatData.Count > 0)
            {
                JavaScriptSerializer js = new JavaScriptSerializer();
                return js.Serialize(BeatData.EsriBeats.beatData);
            }
            else
            {
                return "";
        ...
        public string getBeatDataByBeat(string BeatNumber)
        {
            ...
            var bList = from b in BeatData.EsriBeats.beatData
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(AssignedBeat))
                        {
                            ...
                            BeatData.esriBeat eb = BeatData.EsriBeats.beatPolygons.Find(b => b.BeatID == AssignedBeat);
                            if (eb != null)
                            {
            ...
            if (thisTruck != null)
            {
                ...
                */
                ...
                if (logoffOK == true)
                {
                    ...
                    //end mileage record
                    if (string.IsNullOrEmpty(endODO) || endODO.ToUpper() == "NULL") {
                        endODO = "0.0";
                    }
        ...
        public string GetBeats()
        {
            ...
            foreach (BeatData.esriBeat eb in BeatData.EsriBeats.beatPolygons)
            {
                MiscData.ClientBeat thisClient = new MiscData.ClientBeat();
                thisClient.BeatID = new Guid("00000000-0000-0000-0000-000000000000");
                thisClient.BeatName = eb.BeatID;
                if (eb.BeatID != "NO BEAT ID")
                {
        ...
        public string getDropsites(string BeatNumber)
        {
            ...
            drops.Add("Select");
            drops.Add("Other");
            var dsList = from d in BeatData.EsriDropSites.dropPolygons
                         where d.BeatID == BeatNumber
                         select d;
            foreach (BeatData.esriDropSite ds in dsList)
            {
                orderable.Add(ds.Name);
            }

...
public class ReceiveSocket extends TrackerThread implements IRunStateListener {
	...
	private void processRequest(Hashtable request, String xml) {
		try {
			...
			if (command.equals(CMD_SURVEY)) {
				logWrite("processRequest processed " + CMD_SURVEY);
				try {
					int startIndex = command.length() + 2;
					int endIndex = xml.length() - (startIndex + 1);
					String value = xml.substring(startIndex, endIndex);
					logWrite("value " + value);
					irunStateService.setState(CMD_SURVEY, value);
				} catch (Throwable ex) {
			...
			if (command.equals(CMD_SET_VAR)) {
				...
				for (Enumeration enum1 = vRequest.elements(); enum1.hasMoreElements(); ) {
					TagValuePair pair = (TagValuePair) enum1.nextElement();

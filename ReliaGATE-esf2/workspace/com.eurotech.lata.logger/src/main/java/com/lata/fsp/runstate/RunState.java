...
public class RunState extends Thread implements IRunStateService, RunStateConstants {
	...
	public RunState(boolean logging) {
		try {
	...
	private Hashtable getDefaultRunstate(){
		...
		defaultRunstate.put(XML_CELLULAR_SURVEY, "0");
		defaultRunstate.put(XML_DATA_USAGE, "0");
		defaultRunstate.put(XML_SURVEY_STATE, FALSE);
		defaultRunstate.put(XML_IGNITION_TIMEOUT, DEFAULT_TIMEOUT_SECS);
		defaultRunstate.put(XML_BILL_START_DAY, "20");
		defaultRunstate.put(XML_LAST_BILL_RESET, "");
		defaultRunstate.put(XML_IP_HISTORY, "");
		defaultRunstate.put(XML_LOGGED_ON, FALSE);
		defaultRunstate.put(XML_AUTO_LOG_OFF_TIME, "23:00");
		defaultRunstate.put(XML_WIFI_STATE, "0");
	...
	private void loadMTermConfigStateVars() {
		synchronized (lock){
			...
			ftermConfigVars.put(XML_IGNITION_TIMEOUT, getConfigString(XML_IGNITION_TIMEOUT));
			ftermConfigVars.put(XML_BILL_START_DAY, getConfigString(XML_BILL_START_DAY));
			ftermConfigVars.put(XML_AUTO_LOG_OFF_TIME, getConfigString(XML_AUTO_LOG_OFF_TIME));
		}
	}
	...
	public void addRunStateListener(IRunStateListener listener){
		...
		logWrite("addRunStateListener added " + listener.toString());
	}

	public void removeRunStateListener(IRunStateListener listener){
		rsll.removeListener(listener);
		logWrite("removeRunStateListener removed " + listener.toString());
	}

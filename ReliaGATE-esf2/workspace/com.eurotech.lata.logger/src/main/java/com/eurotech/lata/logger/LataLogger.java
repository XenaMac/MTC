...
import com.eurotech.framework.net.modem.ModemMonitorListener;
import com.eurotech.framework.net.modem.ModemMonitorService;
import com.eurotech.framework.net.wifi.WifiClientMonitorListener;
import com.eurotech.framework.net.wifi.WifiClientMonitorService;
...
CellularSurveyConstants, IRunStateListener {
	...
	private void startSurvey() {
		...
		if (netIntrfcAddr != null && netIntrfcAddr.length() > 0) {
			...
			wifiState = new WifiState(runstate, 15, debug);
			if (active) {
				wifiState.start();
			}
		} else {
	...
	private void stopSurvey() {
		...
		dataRecorder = null;
		wifiState.stopThread();
		try {
			wifiState.join();
		} catch (InterruptedException ex) {
			s_logger.info("stopSurvey: problem joining wifiState thread " + ex.toString());
		}
		wifiState = null;
		stopTransferRateSurvey();

...
public class Pinger extends Thread implements CellularSurveyConstants {
  ...
  public Pinger(DataRecorder dataRecorder, int clientIndex, String netIntrfc, Ping ping, int pingUpdateSeconds, boolean logging) {
      ...
      try {
        ...
        if (dataRecorder != null) {
        	try {
        		...
        		dataRecorder.saveResponseTime(clientIndex, pingAverageStr);
        	} catch (Exception ex) {
        		logWrite(" Pinger " + clientIndex + " error writing to data recorder");
        	}
        } else {
          logWrite(" FTPClient " + clientIndex + " error no data recorder");
        }
        try {
